...
def get_batch():
    ...
    with tf.device('/cpu:0'):
        ...
        def get_text_and_spectrograms(_inputs):
            ...
            return _spectrogram_in,_magnitude_in, _spectrogram, _magnitude
            
        # Decode sound file
        x,q, y, z = get_text_and_spectrograms(inputs=[text, sound_file], 
                                            dtypes=[tf.float32,tf.float32, tf.float32, tf.float32],
                                            capacity=128,
                                            num_threads=32)
        
        # create batch queues
        x,q, y, z = tf.train.batch([x,q, y, z],
                                shapes=[(None, hp.n_mels*hp.r), (None, (1+hp.n_fft//2)*hp.r), (None, hp.n_mels*hp.r), (None, (1+hp.n_fft//2)*hp.r)],
                                num_threads=32,
